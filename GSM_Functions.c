...
char Mobile_no[14];
char Mobile_no1[14]="0971550398";                /* store mobile no. of received message */
char message_received[160];        /* save received message */
...
void main(void)
{
    ...
    Intuart();
    dat[4] = 0;   //Flag inicia limpo
    dat[5] = 0;   //Flag inicia limpo
    Display_Init();
    Lcd_Cmd(_LCD_CLEAR);
    ...
    while(1)
    {
             ...
             {   Lcd_Out(1,16,"msg:1");
                 delay_ms(1000);
                 Lcd_Cmd(_LCD_CLEAR);
                 Lcd_Out(1,1,"New message"); /* new message arrived */
                 Delay_ms(1000);
                 Lcd_Cmd(_LCD_CLEAR);
...
void Interrupt()
{
    ...
    if(PIR1.RC1IF)
    {
        ...
        if(RCSTA1.OERR)
        {
            RCSTA1.CREN = 0; // cho phép ti?p nh?n
            NOP();
            RCSTA1.CREN = 1; // cho phép ti?p nh?n
        }
        status_flag=1;
    }
...
void checksms(){
    ...
    GSM_Calling(Mobile_no1); /* call sender of message */
    Lcd_Cmd(_LCD_CLEAR);
    Lcd_Out(1,1,"Calling...");
    Delay_ms(15000);
